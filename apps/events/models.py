...
class Evento(models.Model):
    id = models.BigAutoField(primary_key=True)
    estado_evento = models.ForeignKey(EstadoEvento, on_delete=models.PROTECT)
    nombre = models.CharField(max_length=150)
    descripcion = models.TextField(blank=True, null=True)
    fecha_evento = models.DateField()
    hora_evento = models.TimeField()
    lugar = models.CharField(max_length=200, default='Teatro al Aire Libre Jaime Laredo')
    # Cambiado a URLField para validación automática
    imagen_url = models.URLField(max_length=255, blank=True, null=True)
    # auto_now_add para que Django ponga la fecha sola al crear
...
class EventoAsiento(models.Model):
    ...
    # DISPONIBLE, RESERVADO, BLOQUEADO, VENDIDO
...
class FasePrecioZona(models.Model):
    id = models.BigAutoField(primary_key=True)
    fase_precio = models.ForeignKey(FasePrecio, on_delete=models.CASCADE)
    evento_zona = models.ForeignKey(EventoZona, on_delete=models.CASCADE)
